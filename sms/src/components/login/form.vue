          ...
          <el-col :span="22">
            <el-dropdown trigger="click" v-if="dealCookie.length > 0">
              <el-input v-model="form.username" maxlength="15" @keyup.enter.native="loginDone('form')"
                style="width: 345px" clearable autofocus placeholder="请输入账号"></el-input>
              <!--用户登录过的账号-->
        ...
        methods: {
            ...
            checkLogin (formName) {
              ...
              this.$refs[formName].validate(valid => {
                if (valid) {
                  ...
                  this.loginBtn = true;
                  this.$nextTick(() => {
                    _this.axiosHelper.get('/student_backend/login.do', {params: this.form}).then(
                      response => {
                        this.loginBtn = false;
                        this.loginState = 0;
            ...
            click (_this, data) {
              if(Object.keys(data).length > 0) {
                ...
                let username = data.studentId
                if(data.level === 1){
                  username = data.teacherId
                } else if(data.level === 0){
                  username = data.adminCode
                }
                let obj = {
                  username: username,
                  password: data.password,
                ...
                // 设置cookies
                this.$cookies.set(`sms_${obj.username}`, obj, 60*60*24*10);
                this.$cookies.set(`userSession`, data, 60*60*24*10);
                // localStorage.setItem('userToken', data.token);
                // localStorage.setItem('refreshToken', data.refreshToken);
                localStorage.setItem('cookiesName', `userSession`);
                // 跳转到主页
